...
local temple2 = {
    identifier = "Temple-2",
    title = "[Episode C, Level 5] Pyromancer",
    theme = THEME.TEMPLE,
    world = 3,
	level = 5,
    width = 4,
...
temple2.load_level = function()
	...
	replace_drop(DROP.SORCERESS_RUBY, ENT_TYPE.ITEM_BLOOD)
	...
	level_state.callbacks[#level_state.callbacks+1] = set_pre_entity_spawn(function(type, x, y, l, overlay)
		return spawn_entity_nonreplaceable(ENT_TYPE.ITEM_FIREBALL, x, y, l, 0, 0)
	end, SPAWN_TYPE.ANY, 0, ENT_TYPE.ITEM_SORCERESS_DAGGER_SHOT)
	...
	local enemy_uid = {}
	local enemies_dead = {}
	lifebar = 2101 --Number of frames the player has left to live
	level_state.callbacks[#level_state.callbacks+1] = set_callback(function ()
		...
		if frames == 0 then
			...
			enemy_uid = get_entities_by_type(ENT_TYPE.MONS_SORCERESS)
		...
		for i = 1,#enemy_uid do
			local sorceress = get_entity(enemy_uid[i])
			if sorceress ~= nil and enemies_dead[i] ~= true then
				sorceress:light_on_fire(2)
			end
		end
		...
		chains = get_entities_by_type(ENT_TYPE.FLOOR_CHAINANDBLOCKS_CHAIN)
		for i = 1,#chains do
			get_entity(chains[i]).animation_frame = 16
		end
...
temple2.unload_level = function()
	...
	replace_drop(DROP.SORCERESS_RUBY, ENT_TYPE.ITEM_RUBY)
